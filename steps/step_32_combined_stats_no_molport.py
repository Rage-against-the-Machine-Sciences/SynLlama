...
def combine_stats(enamine_reconstruct_csv, total_num_mols, llama_folder=None):
    ...
    file_name = enamine_reconstruct_csv[:-4].split("/")[-1].split("_enamine_reconstruct")[0]
    enamine_reconstruct_df.to_csv(os.path.join(llama_folder, f"{file_name}_final_reconstruct_stats.csv"), index=False)
...
if __name__ == "__main__":
    ...
    for enamine_reconstruct_path in enamine_reconstruct_paths:
        ...
        result = combine_stats(enamine_reconstruct_path, total_num_mols=args.total_num_mols, llama_folder=args.llama_folder)
